...
export function normalizeMapaCrop(rawCrop) {
  return {
    top: clamp(Number(rawCrop?.top) || 0, 0, 30),
    right: clamp(Number(rawCrop?.right) || 0, 0, 30),
    bottom: clamp(Number(rawCrop?.bottom) || 0, 0, 30),
    left: clamp(Number(rawCrop?.left) || 0, 0, 30)
  };
}
