          ...
          {MAIN_LINKS.map((item) => (
            <Link
              ...
              className={`flex flex-col items-center justify-center gap-0.5 text-xs font-semibold ${
                isActive(item.href) ? 'text-wine' : 'text-roseDeep/80'
              }`}
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              {EXTRA_LINKS.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`rounded-2xl border px-3 py-2 text-center text-sm font-semibold ${
                    isActive(item.href)
                      ? 'border-wine/30 bg-wine/10 text-wine'
                      : 'border-roseDeep/20 bg-white/80 text-wine'
                  }`}
